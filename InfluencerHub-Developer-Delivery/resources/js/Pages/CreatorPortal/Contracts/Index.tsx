...
interface Props { creatorName: string; items: Paginated<Row>; awaiting: number }

const money = (m: number, cur: string) => (m / 100).toLocaleString('en-US') + ' ' + cur;

export default function CreatorContractsIndex({ creatorName, items, awaiting }: Props) {
  return (
    <AppShell heading="العقود" nav={creatorNav} portal="creator" wsName={creatorName} wsPlan="بوابة المبدع">
      <Head title="العقود" />
      <ListHead eyebrow="بوابة المبدع" title="العقود" sub="راجع عقودك ووقّع ما بانتظار قبولك." />
      ...
      <div className="ih-kpis">
        ...
        <Kpi label="إجمالي العقود" icon="file-text" value={items.total.toLocaleString('en-US')} sub="لديك" />
      </div>
