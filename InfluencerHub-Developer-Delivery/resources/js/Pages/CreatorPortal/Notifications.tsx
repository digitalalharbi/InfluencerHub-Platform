...
export default function CreatorNotifications({ items, unread }: Props) {
  const open = (n: Item) => router.post(u(`/notifications/${n.id}/read`));

  return (
    <AppShell heading="الإشعارات" nav={creatorNav} portal="creator">
      <Head title="الإشعارات" />

      <ListHead eyebrow="بوابة المبدع" title="الإشعارات"
        sub="تنبيهات التعاونات والمحتوى والعقود والمستحقات الخاصة بك"
        actions={unread > 0
      ...
      {items.data.length === 0 ? (
        <div className="ih-dt-wrap"><div className="ih-empty">
          ...
          <div className="ih-empty__text">تصلك هنا تنبيهات ما يحتاج انتباهك: عرض تعاون جديد، تعديل مطلوب على محتوى، أو عقد للتوقيع.</div>
        </div></div>
      ) : (
        <>
