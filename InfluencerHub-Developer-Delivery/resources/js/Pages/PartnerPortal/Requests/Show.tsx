...
interface RequestT {
  ...
  assignee: string | null; description: string | null; client: string | null; createdAt: string | null; dueAt: string | null; isOpen: boolean;
}
...
interface Props { agencyName: string; request: RequestT; comments: Comment[]; history: History[] }

export default function PartnerRequestShow({ agencyName, request, comments, history }: Props) {
  ...
  const send = () => {
    ...
    router.post(u(`/requests/${request.id}/comment`), { body }, {
      preserveScroll: true, onFinish: () => setBusy(false), onSuccess: () => setBody(''),
    });
  };

  return (
    <AppShell heading="طلب خدمة" nav={partnerNav} portal="partner" wsName={agencyName} wsPlan="بوابة الشريك">
      ...
      <WorkspaceHeader
        ...
        meta={[
          ['العميل', request.client ?? '—'], ['النوع', request.typeLabel],
          ['الأولوية', request.priorityLabel], ['المسؤول', request.assignee ?? 'قيد الإسناد'],
        ]}
      />
          ...
          <Sec title="المحادثة" icon="clipboard-check">
            {comments.length === 0 ? (
              ...
              <div style={{ display: 'grid', gap: '.7rem', marginBottom: '1rem' }}>
                {comments.map((cm) => {
                  const mine = cm.authorType === 'partner';
                  return (
            ...
            {request.isOpen ? (
              <div style={{ display: 'flex', gap: '.5rem', alignItems: 'flex-end' }}>
                <textarea value={body} onChange={(e) => setBody(e.target.value)} className="field" rows={2} placeholder="اكتب رسالة…" style={{ flex: 1, resize: 'vertical' }} />
