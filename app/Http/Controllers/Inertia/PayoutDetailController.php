...
{
    /** [action, labelKey (payouts.act_*), tone, input(none|reason|date|reference)]. التسمية تُترجَم في show. */
    private const ACTIONS = [
        'pending' => [['approve', 'approve', 'primary', 'none'], ['cancel', 'cancel', 'danger', 'none']],
        'approved' => [['schedule', 'schedule', 'primary', 'date'], ['cancel', 'cancel', 'danger', 'none']],
        'scheduled' => [['send-to-provider', 'send_provider', 'primary', 'none'], ['cancel', 'cancel', 'danger', 'none']],
        'waiting_for_provider' => [['mark-paid', 'mark_paid', 'primary', 'reference'], ['mark-failed', 'mark_failed', 'danger', 'reason']],
        'failed' => [['schedule', 'reschedule', 'primary', 'date'], ['cancel', 'cancel', 'danger', 'none']],
        'paid' => [], 'cancelled' => [],
    ];
    ...
    private function statementData(Payout $payout): array
    {
        ...
        return [
            'workspace' => Organization::find(TenantContext::organizationId())?->name ?? Brand::name(),
    ...
    private function stmtArtifact(Request $r, Payout $payout, DocumentArtifactService $svc, bool $regenerate = false): ExportJob
    {
        $data = $this->statementData($payout);
        $render = function () use ($payout, $data, $svc, $r) {
            AuditLogger::log('export.generated', $payout, ['type' => self::STMT_TYPE, 'format' => 'pdf'], $payout->tenant_id, $r->user()?->id);
    ...
    private function streamStmt(ExportJob $a, DocumentArtifactService $svc, string $disposition)
    {
        $bytes = $svc->bytes($a);
    ...
    public function pdfPreview(Request $r, Payout $payout, DocumentArtifactService $svc)
    {
    ...
    public function pdfDownload(Request $r, Payout $payout, DocumentArtifactService $svc)
    {
        $this->authorize('view', $payout);
    ...
    public function pdfRegenerate(Request $r, Payout $payout, DocumentArtifactService $svc): RedirectResponse
    {
        $this->authorize('view', $payout);
    ...
    private function stmtMeta(Payout $payout, DocumentArtifactService $svc): array
    {
        $latest = $svc->latest(self::STMT_TYPE, $payout);
    ...
    public function show(Request $r, Payout $payout, DocumentArtifactService $artifacts): Response
    {
        $this->authorize('view', $payout);
        ...
        return Inertia::render('Payouts/Show', [
            ...
            'actions' => array_map(
                fn (array $a) => [$a[0], trans("payouts.act_{$a[1]}"), $a[2], $a[3]],
