...
| ترشيح المؤثرين — تطوير متكامل | ٣ أوضاع عرض (قائمة `ih-reccard` / بطاقات مربّعة `ih-recgrid` / جدول `ih-dt`) بمبدّل `ih-viewtoggle`، ولوحة تفاصيل جانبية `ih-drawer--end` عند النقر: تحليل ملاءمة + نظرة سريعة + مجالات + جدول تسعير بالهامش المحسوب (بيع−تكلفة) + تواصل، مع أزرار تحويل من داخل اللوحة | ✅ محقّق حيًّا (3 أوضاع + Drawer) |
