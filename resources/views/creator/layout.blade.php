...
<head>
    ...
    <title>@yield('title', 'بوابة المبدع') — InfluencerHub</title>
    <link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+Arabic:wght@400;500;600;700&family=IBM+Plex+Sans:wght@400;500;600;700&display=swap" rel="stylesheet">
...
<div class="ih-shell" x-data="{ nav:false }" :class="{ 'nav-open': nav }" @keydown.escape.window="nav=false"><div class="ih-scrim" @click="nav=false"></div>
    <aside class="sidebar" style="display:flex; flex-direction:column; padding:1.2rem 1rem;" @click="nav=false">
        ...
        <nav style="display:flex; flex-direction:column; gap:.15rem; flex:1; overflow-y:auto;">
            @foreach(['dashboard'=>'الرئيسية','profile'=>'ملفي','platforms'=>'المنصات','services'=>'الخدمات والأسعار','portfolio'=>'نماذج الأعمال','mowthooq'=>'موثوق','financial'=>'البيانات المالية','collaborations'=>'التعاونات','content'=>'المحتوى','contracts'=>'العقود','payouts'=>'المستحقات','notifications'=>'الإشعارات'] as $k=>$v)
                <a href="/creator/{{ $k }}" class="nav-link {{ request()->is('creator/'.$k.'*') ? 'active' : '' }}">{{ $v }}</a>
            @endforeach
            <div style="font-size:.7rem; color:var(--text-muted); font-weight:700; padding:.7rem .9rem .3rem;">لاحقًا</div>
            @foreach(['opportunities'=>'الفرص'] as $k=>$v)
                <a href="/creator/{{ $k }}" class="nav-link" style="opacity:.6;">{{ $v }}</a>
            @endforeach
        </nav>
